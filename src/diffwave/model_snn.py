...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x, diffusion_step, conditioner, mem_conv):
    ...
    def forward(self, audio, diffusion_step, spectrogram, num_steps):
        ...
        for step in range(num_steps):
            ...
            for i, layer in enumerate(self.residual_layers):
                x_res, skip, mem_residual[i] = layer(x_res, diffusion_emb, spectrogram_upsampled, mem_residual[i])
                # print(x_res.mean())
        ...
        return torch.stack(output_spikes_list, dim=0).mean(dim=0)