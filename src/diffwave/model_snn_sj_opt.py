...
# GPU 메모리 최적화를 위한 제안:
#
# 1. 자동 혼합 정밀도 (Automatic Mixed Precision - AMP) 사용:
#    아래와 같이 훈련 루프에서 `torch.cuda.amp.autocast`와 `GradScaler`를 사용하면
#    메모리 사용량을 절반 가까이 줄이고 훈련 속도를 높일 수 있습니다.
#
...
#    이 코드에는 그래디언트 체크포인팅이 구현되어 있습니다.
#    `DiffWave` 모델을 생성할 때 `params`에 `use_checkpointing=True`를 추가하여 활성화할 수 있습니다.
#    이는 훈련 속도를 약간 저하하지만, 메모리 사용량을 크게 줄여줍니다.
